            ...
            <section className="bg-card border border-border rounded-[var(--radius)] p-4 shadow-sm">
              <div className="flex items-start justify-between mb-1">
                ...
                <Button
                  ...
                  size="icon"
                  className="h-5 w-5"
                  onClick={() => setShowExceptionsInfo(true)}
                  title="What are priority exceptions?"
                >
                      ...
                      <div key={ex.id} className="border border-border/80 rounded-[var(--radius)] bg-card overflow-hidden">
                        ...
                        {!ex.hasActions && (
                          <div className="p-2 bg-card border-t border-border/40 flex justify-end">
                            <Button variant="ghost" size="sm" className="text-[10px] h-auto">
            ...
            <div className="bg-white rounded-[var(--radius)] shadow-2xl w-full max-w-lg border border-border p-6" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="text-base font-bold text-foreground">Priority Exceptions</h2>
                </div>
                <Button variant="ghost" size="icon" onClick={() => setShowExceptionsInfo(false)}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </Button>
              ...
              <div className="space-y-4 text-xs text-muted-foreground leading-relaxed">
                <p>
                  <strong className="text-foreground">Priority exceptions</strong> are bank transactions that the
                  auto-reconciliation engine could not match to any ledger entry (confidence = 0%).
                </p>
                <p>
                  These unmatched lines typically require <strong className="text-foreground">manual intervention</strong> from a finance
                  user to resolve — for example by:
                </p>
                <ul className="list-disc pl-5 space-y-1">
                  <li>Creating a new journal entry to record the transaction</li>
                  <li>Assigning a category (e.g. Utilities, Office Supplies)</li>
                  <li>Marking it as a personal expense or director loan</li>
                </ul>
                <p>
                  They are flagged as <span className="text-destructive font-semibold">high priority</span> because
                  unreconciled items can cause bank reconciliation to fail, delaying month-end close and
                  producing inaccurate cash positions.
                </p>
              </div>

              <div className="mt-6 flex justify-end">
                <Button variant="default" onClick={() => setShowExceptionsInfo(false)}>
                  Got it
                </Button>
              </div>
            </div>
        ...
        <div className="bg-white rounded-[var(--radius)] shadow-2xl w-full max-w-md border border-border" onClick={(e) => e.stopPropagation()}>
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            <div className="flex justify-end space-x-3 pt-2 border-t border-border">
              ...
              <Button variant="default" type="submit" disabled={connecting || !bankName || !accountNumber}>
