...
export async function seedLeaveData() {
  ...
  // Create Leave Balances for Sara Javed (monthly reset)
  const currentMonth = new Date().getMonth() + 1;
  await prisma.leaveBalance.createMany({
    data: [
      { employeeId: saraEmp.id, year: currentYear, month: currentMonth, leaveType: LeaveType.ANNUAL, allocatedDays: 20, usedDays: 8, pendingDays: 0 },
      { employeeId: saraEmp.id, year: currentYear, month: currentMonth, leaveType: LeaveType.SICK, allocatedDays: 20, usedDays: 2, pendingDays: 0 },
      { employeeId: saraEmp.id, year: currentYear, month: currentMonth, leaveType: LeaveType.CASUAL, allocatedDays: 20, usedDays: 3, pendingDays: 0 },
      { employeeId: saraEmp.id, year: currentYear, month: currentMonth, leaveType: LeaveType.WFH, allocatedDays: 20, usedDays: 6, pendingDays: 0 },
      { employeeId: saraEmp.id, year: currentYear, month: currentMonth, leaveType: LeaveType.UNPAID, allocatedDays: 20, usedDays: 0, pendingDays: 0 },
    ],
  });
